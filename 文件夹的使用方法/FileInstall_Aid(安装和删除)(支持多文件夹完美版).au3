...
#PRE_Icon=G:\Autoit3\辅助工具AU3\FileInstall_Aid.exe
#PRE_Outfile=FileInstall_Aid.exe
#PRE_Res_Comment=小站修改
#PRE_Res_Description=小站修改
#PRE_Res_Fileversion=2.0.0.0
#PRE_Res_LegalCopyright=小站修改
...
Func fxg()
		...
		$TXT = GUICtrlRead($INPUT1)
			...
			For $i = 0 To UBound($aLine) - 1
				...
				$CJMU = $cj & $wjj
				FileWriteLine(@DesktopDir & "\结果.au3", $CJMU)
				$0 = $0 + 1
			Next
			...
			If $TXT <> "" Then
				...
				EndIf
					...
					Sleep(10)
				Next
					FileWriteLine(@DesktopDir & "\结果.au3", ";本脚本由风雪FileInstall函数自动装入工具生成(感谢afan的技术支持)！QQ:61703250，技术群：12467473")
		...
		$TXT = GUICtrlRead($INPUT1)
				...
				If GUICtrlRead($RADIO3) = $GUI_CHECKED Then
					$AFILEXD = _SSTRINGFUNCTIONNAME($AFILE[$I], $FILE2)
						...
						$11 = $10 & $TXT & " & " & '"'
						$12 = $11 & $AFILEXD & '"' & ")"
						FileWriteLine(@DesktopDir & "\结果.au3", $12)
						$0 = $0 + 1
						GUICtrlSetData($LABEL5, "已生成 " & $0 & " 行代码.")
						GUICtrlSetData($PROGRESS1, (($I + 1) / (UBound($AFILE) - 2)) * 100)
						Sleep(10)
				Else
					If 	$I = 0 Then ;根文件夹必须有一个文件才可以
						$AFILEXD = _SSTRINGFUNCTIONNAME1($AFILE[$I], $FILE2)
						$10 = 'DirRemove('
						$11 = $10 & $TXT & " & " & '"\'
						$12 = $11 & $FILE2 & '",' & "1" & ")"
