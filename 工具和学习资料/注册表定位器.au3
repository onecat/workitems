...
#AutoIt3Wrapper_Res_Field=CompanyName|缘鸟脚本工作室
...
$Form1 = GUICreate("注册表定位器            by:大海", 402, 46)
$Edit1 = GUICtrlCreateEdit("", 2, 2, 370, 42, $WS_VSCROLL)
GUICtrlSetData(-1, ClipGet())
$Button1 = GUICtrlCreateButton("跳"&@CRLF&"转", 374, 2, 25, 42,0x2000)
...
While 1
        ...
        Switch $nMsg
                Case $GUI_EVENT_CLOSE
                        Exit
                Case $Button1
                        $sReg = GUICtrlRead($Edit1)
                        RegWrite('HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Applets\Regedit', 'LastKey', 'REG_SZ', $sReg)
                        Run('regedit.exe -m')
        EndSwitch
WEnd