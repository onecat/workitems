...
;====================================================================================
; 说明:　按指定的进程名删除其在系统托盘的图标，同时清除进程已退出而残留的托盘图标。
; 语法:　_SysTrayIcon_Clean(['进程名'[, 托盘类型]])
; 参数:　可选参数：'进程名' - 即在任务管理器中显示的名称(含后缀)。可指定多个进程名，
;　　　　　　　　　多个时以“|”间隔，留空或-1则仅清除进程已退出而残留的托盘图标(默认)。
;　　　　可选参数：'托盘类型' - 同此函数库相关解释：
;　　　　　　　　　0 - ToolbarWindow32, Win2000+；1 - NotifyIconOverflowWindow, Win7+(默认)。
; 返回值:成功 - 返回1。删除图标的数量保存在@extended宏中。
;　　　　失败 - 返回-1：获取图标进程列表错误或删除图标错误。
;　　　　失败 - 返回0：没有找到匹配的图标。
; 作者:　Afan -- http://www.autoit.net.cn
;=====================================================================================
Func _SysTrayIcon_Clean($processes = '', $iWin = 1)
	If $processes = -1 Then $processes = ''
	Local $a_processes = _SysTrayIconProcesses($iWin)
	If $a_processes = -1 Then Return -1
	Local $x = 0, $ret, $i
	For $i = 0 To UBound($a_processes) - 1
		If StringInStr('||' & $processes & '|', '|' & $a_processes[$i] & '|') Then
...
;
;===============================================================================
Func _SysTrayGetButtonInfo($iIndex, $iWin = 1, $iInfo = 2, $iHide = -1)
    ;=========================================================
