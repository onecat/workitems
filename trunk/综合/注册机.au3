...
$zhuce = GUICreate("注册机", 239, 94, 355, 290)
GUICtrlCreateLabel("机器码", 8, 8, 40, 17)
$jqm = GUICtrlCreateInput("", 48, 4, 185, 21)
GUICtrlCreateLabel("注册码", 8, 40, 40, 17)
$zhucema = GUICtrlCreateInput("", 48, 35, 185, 21)
$zc = GUICtrlCreateButton("获得注册码", 80, 64, 75, 25, 0)
GUISetState(@SW_SHOW)
...
While 1
        ...
        Case $zc
                        ...
                        If $a="" Then
                                MsgBox(0,"警告","不能为空")
                        Else
                                GUICtrlSetData($zhucema,md35(_StringDecryptRC4("0x"&GUICtrlRead($jqm), "gto250")))
                        EndIf
...
;===============================================================================
;以下是RC4加密解密代码     
;  _StringEncryptRC4($text, $encryptkey)  加密   
;  _StringDecryptRC4($text, $encryptkey)  解密                                        
;===============================================================================
;
...
Func _StringEncryptRC4($text, $encryptkey)
    ...
    _RC4Initialize($encryptkey, $key, $sbox)
...
; Helper function
Func _RC4Initialize($strPwd, ByRef $key, ByRef $sbox)
    Dim $tempSwap
    Dim $a
    Dim $b
    ...
    For $a = 0 To 255
        ...
        $sbox[$b] = $tempSwap
    Next
EndFunc   ;==>_RC4Initialize

Func _HexToStringEx($strHex)
    Return BinaryToString($strHex)
EndFunc   ;==>_HexToStringEx

Func _StringToHexEx($strChar)
    Return StringToBinary($strChar)
