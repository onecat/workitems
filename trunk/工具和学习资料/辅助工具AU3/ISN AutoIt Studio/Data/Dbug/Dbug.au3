...
Func Dbug($lnr = @ScriptLineNumber, $case = -5, $exp = 0, $exp2 = 0)	;main function
		...
		While True
			...
			Switch $Msg
				...
				Case $DBGListView
					...
					_GUICtrlListView_SortItems($DBGListView, GUICtrlGetState($DBGListView))
	...
	Case -5	;set breakpoint check
		If Ubound($DBGLineFun) = $lnr Then $lnr -= 1
		$DBGlnr = $lnr
		Static $LastFun
		...
		If TimerDiff($DBGtimer) > 250 Then	;update calling function display every 250ms
			If $lnr = 1 then
				GUICtrlSetData($DBGlblStat, StringFormat(_Get_langstr(867), $DBGLineFun[0], $DBGLineFun[$lnr], $lnr)) ; original string: '%s calls, last from %s(%s)'
			Else
				GUICtrlSetData($DBGlblStat, StringFormat(_Get_langstr(867), $DBGLineFun[0], $DBGLineFun[$lnr-1], $lnr-1)) ; original string: '%s calls, last from %s(%s)'
			EndIf
			$DBGtimer = TimerInit()
		...
		$val = _SCISendMessage($DBGhSci, 2046, $lnr-2)		;markerget
		If $CmdLine[0] = 0 Or BitAND($val, 0x0002) = 2 Or $DBGstepMode Or $DBGLineFun[$lnr-1] = $DBGBreakFun Or $lnr = $DBGBreakLine Or $breaknow Then
	...
	Case -2	;init
		...
		if $tmp < 0 then $tmp = 0
		;end
		_SCISendMessage($DBGhSci, 2234, $tmp, 0)	;$SCI_ENSUREVISIBLEENFORCEPOLICY
		_SCISendMessage($DBGhSci, 2024, $tmp)		;gotoline
		_SCISendMessage($DBGhSci, 2045, 3)			;delete markers
		_SCISendMessage($DBGhSci, 2043, $tmp, 3)	;add marker
		...
		If $lnr = 1 then
			WinSetTitle($frmDBUG, 0, StringFormat(_Get_langstr(868), $DBGLineFun[$lnr], $lnr)) ; original string: 'DBUG - Break in %s(%s)'
		Else
			WinSetTitle($frmDBUG, 0, StringFormat(_Get_langstr(868), $DBGLineFun[$lnr-1], $lnr-1)) ; original string: 'DBUG - Break in %s(%s)'
		EndIf
	...
	Case -1 To 1000		;read
		...
		If $case >= _GUICtrlListView_GetItemCount($DBGListView) Then	;all expressions read
			...
			Next
			If $DBGSortMode then _DBGGUICtrlListView_SortItems_NoToggle($DBGListView, 0, 0)
...
Func _SCIGetLine()
	...
	$pos = _SCISendMessage($DBGhSci, 2008) ; $SCI_GETCURRENTPOSITION
	$line = _SCISendMessage($DBGhSci, 2166, $pos) ; $SCI_LINEFROMPOSITION
	$sta = _SCISendMessage($DBGhSci, 2167, $line) ; $SCI_POSITIONFROMLINE
	$end = _SCISendMessage($DBGhSci, 2136, $line) ; $SCI_GETLINEENDPOSITION

	For $c = $sta To $end
		$char = chr(_SCISendMessage($DBGhSci, 2007, $c, 0)); $SCI_GETCHARAT
		$text &= $char
	Next
	Return $text
EndFunc
...
Func _SCIGetCurWord() ;get current word under cursor from Scite
	...
	$pos = _SCISendMessage($DBGhSci, 2022, $pos[0], $pos[1]) ; $SCI_POSITIONFROMPOINT
	$line = _SCISendMessage($DBGhSci, 2166, $pos) ; $SCI_LINEFROMPOSITION
	$sta = _SCISendMessage($DBGhSci, 2167, $line) ; $SCI_POSITIONFROMLINE
	$end = _SCISendMessage($DBGhSci, 2136, $line) ; $SCI_GETLINEENDPOSITION
...
Func PopulateListView($lnr) ;populate listview
	Local $fx, $CurExpr, $i
		...
		For $i = 0 To UBound($CurExpr)-1
			_GUICtrlListView_AddItem($DBGListView, $CurExpr[$i],-1,$i - 1 + 1000)
		Next
EndFunc
...
$TestPath = _PathSplit(@ScriptFullPath, $szDrive, $szDir, $szFName, $szExt)
	...
	$fOut = FileOpen($szDrive&$szDir&$szFName&"_debug"&$szExt, 2)	;open shadow script
	...
	$params = StringFormat('"%s" "%s" "%s" "%s"', StringReplace(@ScriptName,"_tmp",""), @ScriptDir, $DBGnotifyFun, $DBGcommandFun)
	_FileCreate($szDrive&$szDir&$szFName&"_debug.txt")
	_FileWriteToLine($szDrive&$szDir&$szFName&"_debug.txt", 1, $params, 1)
...
Func DBGShow()	;let's show the thing
	...
	_GUICtrlListView_RegisterSortCallBack($DBGListView)
...
Func DBGExit()	;seen enough now
	_GUICtrlListView_UnRegisterSortCallBack($DBGListView)
	_SCISendMessage($DBGhSci, 2045, 3)	;delete markers
	Exit
EndFunc
...
func _Get_langstr($str)
	$get = iniread($DBGISNDir & "\data\language\" & $DBGLanguagefile, "ISNAUTOITSTUDIO", "str" & $str, "#error#")
	;falls string nicht gefunden...fallback zur englischen sprache
	if $get = "#error#" Then
		$get = iniread($DBGISNDir & "\data\language\english.lng", "ISNAUTOITSTUDIO", "str" & $str, "#LANGUAGE_ERROR#ID#" & $str)
	endif
	$get = StringReplace($get, "[BREAK]", @crlf)
	return $get
EndFunc

; sort listview, but don't change sort order

Func _DBGGUICtrlListView_SortItems_NoToggle($HWND, $iCol, $nSortDir = 0) ;$nSortDir: 1=Ascending, -1=Descending, 0=Current
	...
	Return $iRet <> 0
EndFunc   ;==>_DBGGUICtrlListView_SortItems_NoToggle
