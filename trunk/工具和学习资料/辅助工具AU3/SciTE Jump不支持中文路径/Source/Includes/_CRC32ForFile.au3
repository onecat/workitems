...
; Return values .: Success - Returns CRC32 value in thd form of a hex string
;                          - Sets @error to 0
...
; Example .......: No
; ===============================================================================================================================
Func _CRC32ForFile($sFilePath)
	Local $iError = 0
	Local $hFilePath = _WinAPI_CreateFileEx($sFilePath, $OPEN_EXISTING, $GENERIC_READ, BitOR($FILE_SHARE_READ, $FILE_SHARE_WRITE), $SECURITY_ANONYMOUS, 0, 0)
	If @error Then
		Return SetError(1, 0, "")
	EndIf

	Local $hFilePathMappingObject = _WinAPI_CreateFileMapping($hFilePath, 0, 0, $PAGE_READONLY)
	$iError = @error
	...
	Local $pFilePath = _WinAPI_MapViewOfFile($hFilePathMappingObject, 0, 0, $FILE_MAP_READ)
	$iError = @error
	...
	Local $iCRC32 = _WinAPI_ComputeCrc32($pFilePath, $iBufferSize)
	$iError = @error
