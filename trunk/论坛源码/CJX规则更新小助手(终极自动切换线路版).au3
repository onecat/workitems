...
#PRE_Icon=J:\ADMuncher最终汉化版\CJX规则更新小助手.exe
#PRE_Outfile=C:\Users\chtyfox\Desktop\CJX规则更新小助手 V4.4 by xiaozhan.exe
#PRE_Compression=4
#PRE_Res_Comment=小站制作 by xiaozhan
#PRE_Res_Description=小站制作 by xiaozhan
#PRE_Res_Fileversion=4.4.1.0
...
$qt = HotKeySet("^3", "ExitScript")
Opt("TrayIconHide", 0)
Opt("TrayMenuMode", 1 + 2) ;没有默认的（暂停脚本和退出）菜单. 
...
$Form1 = GUICreate("CJX规则更新小助手", 336, 195, -1, -1, $WS_SYSMENU)
...
$Label2 = GUICtrlCreateLabel("", 104, 24, 104, 17)
$Label3 = GUICtrlCreateLabel("本地规则版本号：", 8, 72, 100, 17)
GUICtrlSetColor($Label3, 0x6633CC)
$Label4 = GUICtrlCreateLabel("", 120, 72, 84, 17)
$Button1 = GUICtrlCreateButton("立即更新", 8, 108, 73, 33,$WS_GROUP)
GUICtrlSetState(-1, $GUI_DEFBUTTON)
$Button2 = GUICtrlCreateButton("清理浏览器缓存", 95, 108, 95, 33)
$Label5 = GUICtrlCreateLabel("程序制作 by xiaozhan", 208, 124, 122, 17)
;Global $iClose = GUICtrlCreateButton("N M 都什么破网速", 199, 185, 132, 20)
GUICtrlSetColor($Label5, 0xFF00FF)
;-----------------------------------------------------------------------------
PN()
$Pic = GUICtrlCreatePic("", 208, 8, 121, 113, $WS_EX_TRANSPARENT)
...
$txsck = TrayCreateItem("显示窗口"  &  "           Ctrl+W") ;创建第三个菜单项
...
Func _tp()
	...
	TrayItemSetOnEvent($tljgx, "ljgx")
	TrayItemSetOnEvent($tyctptb, "yctptb") ;隐藏托盘
	TrayItemSetOnEvent($txsck,"xianshi") 
...
If Not StringRegExp($strgz,$azz) Then;检测代理问题
		GUICtrlSetData($Label2, "在线代理网址失效")
		GUICtrlSetColor($Label2, 0x3399FF)
		Sleep(2*1000)
Else
...
If Not StringInStr($return,"Xlist version") Then
		GUICtrlSetData($Label2, "固定IP代理失败")
		GUICtrlSetColor($Label2, 0x3399FF)
		Sleep(3*1000)
		DLGXBY()
Else	
...
Func Dl();必须带有CJX的wget.exe这个文件
	...
	ElseIf $iPing4 Then
		...
		ddcxgx()
	Else
        GUICtrlSetData($Label2, "CJX代理更新失败")
		GUICtrlSetColor($Label2, 0x3399FF)
		FileCopy(@ScriptDir & "\temp.dat", @ScriptDir & "\CustomStrings.dat", 1)
		FileDelete(@ScriptDir & "\temp.dat")
		Sleep(3*1000)
		GUICtrlSetData($Label2, "等待程序更新")
		GUICtrlSetColor($Label2, 0x3399FF)
    EndIf
EndFunc	
