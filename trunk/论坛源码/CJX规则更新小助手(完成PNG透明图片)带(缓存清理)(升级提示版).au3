...
$Button5 = GUICtrlCreateButton("在线代理更新CJX规则", 205, 3, 122, 20)
...
$Label8 = GUICtrlCreateLabel("网络规则版本号：", 8, 5, 100, 17)
GUICtrlSetColor(-1, 0x008080)
GUICtrlSetState ( $Label8, $GUI_HIDE)
ControlHide("CJX规则更新小助手","网络规则版本号：",$Label8)
$Label9 = GUICtrlCreateLabel("", 120, 5, 80, 17)
...
Func PBNN();屏蔽按钮为灰色状态不可点击
	...
	_GUICtrlButton_Enable($Button5,False)
...
Func XXAN();显示之前屏蔽的按钮
	...
	_GUICtrlButton_Enable($Button4,True)
	_GUICtrlButton_Enable($Button5,True)
...
Func ZXDLGZ()
$oHTTP = ObjCreate("winhttp.winhttprequest.5.1")
$oHTTP.SetProxy(2,getProxy1());设置代理
$oHTTP.Open("GET",'http://cjxlist.googlecode.com/svn/CustomStrings.dat') ;发送
$oHTTP.Send()
$return = $oHTTP.responsetext
If Not StringInStr($return,"Xlist version") Then
...
EndFunc	
 ...
 Func getProxy1()
$azz="<td>(\S+)</td>[^.]+<td>(\d+)</td>"
$strgz=GetSource2("http://www.xici.net.co/wn/");http://www.xici.net.co/wt/
...
;查看规则是否有新的--------------------------------------------------------------------------------------------------------------------
Func wlgxyl()
		$a = GetSourced1("http://cjxlist.googlecode.com/svn/CustomStrings.dat")
...
Func bbgzxbb();判断网络CJX规则和本地CJX规则 
	_GUICtrlButton_Enable($Button1,False)
	_GUICtrlButton_Enable($Button2,False)
	_GUICtrlButton_Enable($Button3,False)
	_GUICtrlButton_Enable($Button4,False)
	_GUICtrlButton_Enable($Button5,False)
	$HWL = _StringToHex(wlgxyl())
	$HBD = _StringToHex(BDCJXGZ())
	If $HWL <= $HBD Then
		GUICtrlSetState ( $Label8, $GUI_HIDE)
		GUICtrlSetState ( $Label9, $GUI_HIDE)
	...
	Else	
		...
		GUICtrlSetColor($Label8, 0x008080)
		GUICtrlSetState ( $Label9, $GUI_SHOW)
		GUICtrlSetData($Label9,	wlgxyl())
