...
#AutoIt3Wrapper_outfile=获取回放列表总数(Netviewer).exe
...
$I= IniRead(@scriptdir & "\获取回放列表总数(Netviewer).ini", "Netviewer","标题","")
...
While 1
	...
	Select 
		...
		Case $nMsg = $Button2
			Exit
		Case $nMsg = $Button1
			IniWrite(@ScriptDir & "\获取回放列表总数(Netviewer).ini", "Netviewer", "标题", GUICtrlRead ($Input1)) 
			_hqzs()
			Exit
	EndSelect
WEnd
...
Func _hqzs()
	...
	IniWrite(@ScriptDir & "\获取回放列表总数(Netviewer).ini", "列表", "当天总数", $count);单独一天的数目总数
	;$R = IniRead(@ScriptDir & "\获取回放列表总数(Netviewer).ini", "列表", "相加总数和","");读取相加后的总数和
	;IniWrite(@ScriptDir & "\获取回放列表总数(Netviewer).ini", "列表", "相加总数和", $count + $R) ;需要把单独一天的数目总数 + 相加后的总数
	;$Z = IniRead(@ScriptDir & "\获取回放列表总数(Netviewer).ini", "列表", "相加总数和","");$count+$H后的总数
